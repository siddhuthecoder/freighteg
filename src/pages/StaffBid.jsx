...
const StaffBid = () => {
  const user = useSelector((state) => state.login.user); // assuming this is how you get the user id
  ...
  useEffect(() => {
    if (bids.length > 0) {
      ...
      if (assignedTransporter?.length > 0) {
  ...
  useEffect(() => {
    if (activeBidsData?.data) {
      let responsedCount = 0;
      let unrespondedCount = 0;
      activeBidsData?.data.forEach((bid) => {
  ...
  const filteredBidDetails =
    activeBidsData?.data?.filter(
      (bid) =>
  ...
  if (activeBidsLoading) return <div>Loading...</div>;
  if (activeBidsError && activeBidsFetchError) return <div>Error:</div>;
          ...
          <div className="flex gap-5 items-center">
            <div className="bg-gray-100 rounded-lg shadow p-2">
              <p className="text-sm font-semibold min-w-5 min-h-5">
                Live ({activeBidsData?.data?.length || 0})
              </p>
            </div>
            <div className="bg-gray-100 rounded-lg shadow p-2">
            ...
            <table className="min-w-full divide-y divide-gray-200 text-center ">
              <thead>
                <tr className="bg-blue-900 text-white">
                  ...
                  <th className="py-3 px-4 text-left">Bid remaining time</th>
              ...
              <tbody>
                {filteredBidDetails.length > 0 ? (
                  filteredBidDetails.map((data, index) => {
                    const createdByUserData = getVendorData?.find(
