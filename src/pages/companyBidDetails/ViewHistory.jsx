...
const ViewHistory = ({ id }) => {
  ...
  const { vendorDetail, HistoryBidData } = useHistoryData();
  const [vendorData, setVendorData] = useState([]);
  useEffect(() => {
    const resultBid = HistoryBidData?.find((bid) => bid.bid_id === id) || [];
    if (resultBid) {
        ...
        <table className="min-w-full">
          <thead>
            {vendorData && vendorData.length > 0 ? (
              <tr className="text-[#888888] text-left" colspan={12}>
          ...
          {vendorData && vendorData.length > 0 ? (
            vendorData?.map((item, index) => {
              ...
              const resultBid =
                HistoryBidData?.find((bid) => bid.bid_id === id) || [];
              return (
