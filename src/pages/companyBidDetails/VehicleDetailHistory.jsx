...
import { useHistoryData } from "../../HelperFunction/api";
const VehicleDetailHistory = ({ rowData }) => {
  const { HistoryBidData } = useHistoryData();
  const [vendorData, setVendorData] = useState([]);
  useEffect(() => {
    if (HistoryBidData && rowData._id) {
      const ResponseItem = HistoryBidData.map((curr) => curr).filter(
        (Current) => rowData._id.includes(Current.bid_id)
      );
      setVendorData(ResponseItem);
    }
  }, [HistoryBidData, rowData._id]);
