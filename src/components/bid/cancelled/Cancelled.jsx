...
const Cancelled = () => {
  ...
  const fetchBidIdsAndDetails = async () => {
    ...
    const url =
      branchId && branchId !== user?.id
        ? `https://freighteg.in/freightapi/cancelledBids?branch_id=${branchId}&page=${currentPage}&limit=5`
        : `https://freighteg.in/freightapi/cancelledBids?company_id=${user?.id}&page=${currentPage}&limit=5`;
    try {
      // debugger;
      console.log({ url });
      // alert(url)
  ...
  useEffect(() => {
    ...
    async function getCount() {
      const [cancelledRes] = await Promise.all([fetch(`${url}`)]);

      const CancelledData = await cancelledRes.json();
      // alert(CancelledData.totalBids)
      settotalPages(Math.ceil(CancelledData.total / itemsPerPage));
    }
    getCount();
    getAllBidDetails();
  }, [currentPage]);
